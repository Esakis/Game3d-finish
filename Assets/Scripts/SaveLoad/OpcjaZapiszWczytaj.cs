...
using UnityEngine.UI; //Do operacji na plikach (czytanie, pisanie do pliku).
...
public class OpcjaZapiszWczytaj : MonoBehaviour {
    ...
    void Start() {
        ...
        enemy = EnemyInstance.instancja.GetComponent<Transform>();
    }
    ...
    */
    public void zapisz() {
        ...
        dane.nazwaSceny = SceneManager.GetActiveScene().name; //Pobieramy nazwę poziomu/sceny.
        dane.zdrowie = gracz.GetComponent<detectHit>().healtbar.value;//Pobieramy stan zdrowia.
        ...
        dane.enemyPozycja = new Vector3Serialization(enemy.GetComponent<Transform>().position);
    ...
    */
    public void wczytaj() {
        ...
        if (File.Exists(Application.persistentDataPath + "/graczInfo.data")) {
            ...
            SceneManager.LoadScene(dane.nazwaSceny);//Wczytujemy scenę.            
            gracz.GetComponent<detectHit>().healtbar.SetValueWithoutNotify(dane.zdrowie);//Ustawiamy zdrowie.
            ...
            enemy.GetComponent<Transform>().position = dane.enemyPozycja.pobierzWektor();
